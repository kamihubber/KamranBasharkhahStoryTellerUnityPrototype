...
{
    private List<ActConfig> _currentInteractionChain = new List<ActConfig>();
    //think , experience, ...
        ...
        GameEntity otherCharacterEntity)
    {
        ...
        if (self.MentalState == MentalState.Normal)
        {
            ...
            //2 - my traits + simple random (mistake , ...)
              ...
              //warning : loop stuck crash fix for now ! added a list(chain) of acts being called in chain  to provide skills for aa top act
              // and exclude repeated ones to avoid a loop
              candidateAct = candidateActs.Find(ca => ca != null && !_currentInteractionChain.Contains(ca));
              //candidateAct = candidateActs.Find(ca => ca != null );

              _currentInteractionChain.Add(candidateAct);
              return candidateAct;
    ...
    public void ResetCurrentChain()
    {
        _currentInteractionChain.Clear();
    }
        ...
        GameEntity otherCharacterEntity, SkillComp? seekingComp)
    {
        ...
        foreach (var act in characterActs)
        {
            ...
            other = otherCharacterEntity;

            // //todo  : temp , owner for interact
            // if ((otherCharacterEntity.Owner != null) && (act.ShouldEngageSubjectOwner))
            // {
            //     other = otherCharacterEntity.Owner;
            // }
            // else
            //     other = otherCharacterEntity;
            ...
            {
                if (!other.Skills.Any(skl => skl.skillType == rSkill.skillType))
                {
